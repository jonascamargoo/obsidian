...
Para combinar os dados de várias tabelas, é necessário definir critérios de associação, conhecidos como junções. As junções entre tabelas podem ser realizadas usando as cláusulas WHERE ou JOIN. As tabelas devem ser associadas por meio de chaves primárias e estrangeiras. Os principais tipos de JOIN são: INNER JOIN, LEFT JOIN, RIGHT JOIN, FULL JOIN e CROSS JOIN.

A cláusula INNER JOIN permite combinar dados de várias tabelas com base na relação entre elas. Utilizando essa cláusula, é possível retornar linhas (registros) apenas quando há correspondência em ambas as tabelas (Figura). O formato básico dessa cláusula e possui a seguinte sintaxe: 
...
Ou seja, relacionamos PK e FK a fim de realizar uma interseção:

	![[Pasted image 20240814130357.png]]
